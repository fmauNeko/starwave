...
**Services** (in `src/discord/music/youtube/`):
...
| YouTube session   | `src/discord/music/youtube/innertube-session.service.ts` |
| YouTube streaming | `src/discord/music/youtube/youtube-stream.service.ts`    |
...
| Find                  | Pattern                                         | Scope                        |
| --------------------- | ----------------------------------------------- | ---------------------------- |
| YouTube streaming     | `InnertubeSessionService\|YouTubeStreamService` | `src/discord/music/youtube/` |
...
| Issue                                                 | Cause                                        | Fix                                                                                                                    |
| ----------------------------------------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| YouTube streaming fails on startup                    | InnertubeSessionService failed to initialize | Check logs for `innertube.session.init` or `innertube.session.refresh.failed`; may be a network issue reaching YouTube |
| Age-restricted video fails                            | PoToken-only mode has no cookie path         | Expected behavior; age-restricted content is not supported                                                             |
| LOGIN_REQUIRED / "confirm you're not a bot" on server | Datacenter IP bot detection                  | Set youtube.cookiesPath to a burner-account cookies.txt (see YouTube Authentication); or use a residential proxy       |
| `youtube.cookies` key in config.json                  | Stale key from old yt-dlp config             | Use `youtube.cookiesPath` (a file path) instead; rotate any real cookies/tokens in that file                           |
